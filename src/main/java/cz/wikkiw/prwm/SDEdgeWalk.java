...
public class SDEdgeWalk {
    ...
    public void walk(int dimension){
        ...
        int edgedDim = dimension;
        double start = (rnd.nextDouble() * this.boundary.getRange()) + this.boundary.getMin();
        double end;
        if(start < (this.boundary.getMin() + this.boundary.getRange()/2.0)) {
            end = start + difference*this.boundary.getRange();
        } else {
            end = start - difference*this.boundary.getRange();
        }
...
//        System.out.println("BaseDiff: " + baseDiff);
    ...
    public static void main(String[] args) {
        ...
        for(FitnessFunction ff : list){
            ...
            try {
                writer = new PrintWriter(ff.getName()+"-SD-cut.txt", "UTF-8");
